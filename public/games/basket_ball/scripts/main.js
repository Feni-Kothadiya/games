	...
	_OnEventFromRuntime(b) {
		...
		if (b = runtimeEventHandlers.get(c))
			if (b = b.get(d)) {
				...
				if (null === f) return h;
				h && h.then ? h.then(g => this._PostResultToRuntime(f,
					!0, g)).catch(g => {
...
window.C3_Is_Supported && (window.c3_runtimeInterface = new self.RuntimeInterface({
	...
	projectScripts: [],
	mainProjectScript: "",
	scriptFolder: "scripts/index.html",
...
self.AudioDOMHandler = class extends self.DOMHandler {
	...
	async _AddEffect(a) {
		...
		if ("convolution" === b) try {
			e = await this._GetAudioBuffer(a.bufferOriginalUrl, a.bufferUrl, a.bufferType,
				!1)
		} catch (f) {
...
self.RuntimeInterface.AddDOMHandlerClass(class extends self.DOMHandler {
	...
	async _OnRequestPersistent() {
		try {
			...
			return console.error("[Construct] Error requesting persistent storage: ",
				a), {
				isOk: !1
			}
		}
